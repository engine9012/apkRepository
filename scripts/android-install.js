...
module.exports = function (context) {
    ...
    var targetDir  = path.join(projectRoot, "platforms", "android","src","org","apache","cordova","camera");
	var targetFiles = ["CameraLauncher.java"];
    ...
    if (['after_plugin_add', 'after_plugin_install', 'after_platform_add'].indexOf(context.hook) === -1) {
		...
		targetFiles.forEach(function(file){
			...
			fs.readFile(targetFile, {encoding: 'utf-8'}, function (err, data) {
				...
				data = data.replace("__ANDROID_PACKAGE__", packageName + '.fileprovider.image');
